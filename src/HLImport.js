...
export function getOrMakeClassRowID(featIDList, name) {
	...
	const attrMatch = _.find(attrObjs, function (currentAttrObj) {
		...
		// Eliminate anything in parentheses, dice expressions, and "x#"
		// (we use that to indicate we've taken a feat more than once) before comparing names
		name = name
			.replace(/\(.+\)/g, "")
			.replace(/\d+d\d+(\+\d*)*/g, "")
			.replace(/\+\d+/g, "")
...
export function nameIsShield(name) {
	if (
		name.toLowerCase().indexOf("shield") !== -1 ||
		name.toLowerCase().indexOf("buckler") !== -1 ||
		name.toLowerCase().indexOf("klar") !== -1
	)
		return true;
	return false;
}
...
export function importAttacks(meleeAttacks, rangedAttacks, strBonus, baseAttackBonus) {
	...
	getSectionIDs(repeatPrefix, function (idarray) {
		const weaponNameAttrs = _.union(
			_.map(idarray, function (id) {
				return `${repeatPrefix}_${id}_name`;
			})
		);
		getAttrs(weaponNameAttrs, function (names) {
			const weaponIDList = _.object(
				_.map(names, function (name, attr) {
					return [attr.substring(repeatPrefix.length + 1, attr.indexOf("_name")), name];
				})
			);
			const weaponsList = [];
			const attrs = {};
			const meleeNames = _.map(meleeAttacks, function (obj) {
				return obj._name;
			});
			const rangedNames = _.map(rangedAttacks, function (obj) {
				return obj._name;
			});
			...
			const {naturalAttackRegExp} = PFDB;
			const {primaryNaturalAttacksRegExp} = PFDB;
			...
			if (/^\s*$/.test(meleeNames) !== true) {
				_.each(meleeAttacks, function (v) {
					...
					// check if this is a weapon
					const weaponCompareName = v._name;
					// grabs attack name up to a parenthesis
					const shortNameReg = v._name.match(/^[^(]*/g);
					const shortName = SWUtils.trimBoth(shortNameReg);
					let attackName = "";
					...
					let damageBonus = 0;
					let damageMult = 1;
					const weaponObj = meleeAttacks[_.indexOf(meleeNames, weaponCompareName)];
					const enhancementBonus = parseNum(weaponObj._name.match(/\+\d+/));
					// how many attacks?  count pluses in the attack string
					const iterativeAttacks = parseNum((weaponObj._attack.match(/\+/g) || []).length);
					const weaponDice = weaponObj._damage.match(/\d+d\d+/);
					...
					const weaponAttack = parseNum(weaponObj._attack.match(/^.+?(?=\+)/));
					...
					if (weaponObj._damage !== "As Spell") {
						...
						// masterwork ?
						if (
							v._name.toLowerCase().indexOf("masterwork") !== -1 ||
							_.intersection(masterworkWords, v._name.toLowerCase().split(" ")).length > 0
						) {
						...
						if (!_.isUndefined(weaponObj._attack)) {
							...
							if (iterativeAttacks > 1) {
								...
								for (i = 1; i < iterativeAttacks; i++) {
									iterativeCount = i + 1;
									attrs[
										`${repeatPrefix}_toggle_iterative_attack${iterativeCount}`
									] = `@{var_iterative_attack${iterativeCount}_macro}`;
			...
			if (/^\s*$/.test(rangedNames) !== true) {
				_.each(rangedAttacks, function (v) {
					...
					const weaponCompareName = v._name;
					const weaponObj = rangedAttacks[_.indexOf(rangedNames, weaponCompareName)];
					if (weaponObj._damage !== "As Spell") {
...
export function importSpellClasses(attrs, spellclasses, classes, abScores) {
	...
	for (i = 0; i < spellclasses.length; i++) {
		...
		const spellClassName = spellClass._name
			.replace(/\(([^\)]*)\)/g, "")
			.replace("(", "")
			.replace(")", "")
			.trim();
...
export function importCharacter(characterObj) {
	...
	if (!_.isUndefined(characterObj.classes.class)) {
		...
		if (!_.isUndefined(characterObj.spellclasses.spellclass)) {
			...
			spellClasses = importSpellClasses(
				...
				classes,
				characterObj.attributes.attribute
			);
	...
	const attacks = arrayify(characterObj.melee.weapon || {}).concat(arrayify(characterObj.ranged.weapon || {}));
	const meleeAttacks = arrayify(characterObj.melee.weapon || {});
	const rangedAttacks = arrayify(characterObj.ranged.weapon || {});
	const strBonus = parseNum(characterObj.attributes.attribute[0].attrbonus._modified);
	const baseAttackBonus = parseNum(characterObj.attack._baseattack);
	...
	const bab = parseNum(characterObj.attack._baseattack);
