...
export function updateInitiative (callback, silently,force) {
	...
	getAttrs(['nodex-toggle'],function(v){
		if (parseInt(v['nodex-toggle'],10)) {
			...
			SWUtils.updateRowTotal(["init", "init-trait", "init-misc-mod","checks-cond","buff-initiative-total"], 0, ["condition-Deafened"], false, callback, silently, force);
		} else {
			SWUtils.updateRowTotal(["init", "init-ability-mod", "init-trait", "init-misc-mod","checks-cond","buff-initiative-total"], 0, ["condition-Deafened"], false, callback, silently, force);
		}
	});
}
